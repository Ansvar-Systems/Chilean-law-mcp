/**
 * search_eu_implementations — Search EU directives/regulations referenced by Chilean legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_chilean_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as chilean_statute_count
  ...
  if (input.has_chilean_implementation) {
    sql += ' HAVING chilean_statute_count > 0';
  }
