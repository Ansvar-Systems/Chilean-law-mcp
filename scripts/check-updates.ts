#!/usr/bin/env tsx
/**
 * Chilean Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.bcn.cl/leychile';
const PORTAL_NAME = 'Biblioteca del Congreso Nacional (LeyChile)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/chilean-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Chilean Law MCP — Data Freshness Check');
